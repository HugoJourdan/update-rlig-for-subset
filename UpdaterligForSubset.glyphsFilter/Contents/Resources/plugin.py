...
def get_auto_subsetted_axes(instance):
    ...
    if instance.customParameters["Disable Masters"]:
        ...
        axes_kept = [
            axis.axisTag for i, axis in enumerate(font.axes) if len(axis_values[i]) != 1
        ]
    ...
    def filter(self, layer, inEditView, customParameters):
        ...
        for instance in font.instances:
            if instance.type == 1 and instance.active:
                ...
                if (
                    instance.customParameters["Disable Masters"]
                    and customParameters[0] == "auto"
                ):
                    myTags = get_auto_subsetted_axes(instance)
                elif instance.customParameters["Disable Masters"]:
                    myTags = axis_list
                else:
                    myTags = [axis.axisTag for axis in font.axes]

                # Function to extract tags from a condition line, excluding the word "condition"
                def extract_tags(condition):
                    return [
                        tag
                        for tag in re.findall(r"[a-zA-Z]+", condition)
                        if tag != "condition"
                    ]
